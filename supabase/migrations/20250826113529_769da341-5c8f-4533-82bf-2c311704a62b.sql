...
-- Create similar policy for apply_students using country name matching
...
USING (
  ...
  OR
  -- Staff can only see applicants from their assigned country (match by country name)
  EXISTS (
    SELECT 1 FROM user_sessions us
    JOIN users u ON us.user_id = u.id
    JOIN countries c ON u.country_id = c.id
    ...
    AND u.role IN ('staff', 'finance', 'hostel_team')
    AND u.country_id IS NOT NULL
    AND apply_students.country = c.name
  )
  OR
