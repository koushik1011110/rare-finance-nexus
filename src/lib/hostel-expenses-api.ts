...
export interface HostelExpense {
  ...
  amount: number;
  expense_date: string;
  category: 'Rent' | 'Utilities' | 'Maintenance' | 'Food' | 'Staff' | 'Supplies' | 'General';
...
export interface HostelExpenseFormData {
  id?: number;
  hostel_id: string;
  expense_type: string;
  description: string;
...
export const hostelExpensesAPI = {
  getAll: async (): Promise<HostelExpense[]> => {
    ...
    if (error) {
      console.error('Error fetching hostel expenses:', error);
      throw error;
    }
  ...
  create: async (expenseData: Omit<HostelExpenseFormData, 'id'>): Promise<HostelExpense> => {
    const { data, error } = await supabase
      .from('hostel_expenses')
      .insert([{
        hostel_id: parseInt(expenseData.hostel_id),
    ...
    if (error) {
      console.error('Error creating hostel expense:', error);
      throw error;
    }
  ...
  update: async (id: number, expenseData: Partial<Omit<HostelExpenseFormData, 'id'>>): Promise<HostelExpense> => {
    ...
    if (error) {
      console.error('Error updating hostel expense:', error);
      throw error;
    }
  ...
  delete: async (id: number): Promise<void> => {
    ...
    if (error) {
      console.error('Error deleting hostel expense:', error);
      throw error;
    }
  },
};
